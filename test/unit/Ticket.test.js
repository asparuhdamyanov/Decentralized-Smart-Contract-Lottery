...
describe("Ticket", async function () {
  beforeEach(async function () {
    ...
    await Promise.all([
      this.WINNER_PICKER_MOCK.mock.LINK_TOKEN.returns(
        this.LINK_TOKEN_MOCK.address
      ),
      this.WINNER_PICKER_MOCK.mock.fee.returns(ethers.utils.parseEther("0.25")),
      this.WINNER_PICKER_MOCK.mock.getRandomNumber.returns(
        "0x" + Array(65).join("0")
      ),
      this.LINK_TOKEN_MOCK.mock.transferFrom
        .withArgs(
          deployer.address,
          this.WINNER_PICKER_MOCK.address,
    ...
    this.PARAMS = [
      NAME,
      SYMBOL,
      this.BLOCKS.START_BLOCK,
      this.BLOCKS.END_BLOCK,
      PRICE,
      this.WINNER_PICKER_MOCK.address,
    ];
  });
  ...
  describe("Upon initialization", async function () {
    describe("Setting properties", async function () {
      ...
      it("should set correct start block", async function () {
        expect(await this.Ticket.START_BLOCK()).to.equal(
          this.BLOCKS.START_BLOCK
        );
      });

      it("should set correct end block", async function () {
        expect(await this.Ticket.END_BLOCK()).to.equal(this.BLOCKS.END_BLOCK);
      });
      ...
      it("should set correct winner picker / vrf consumer", async function () {
        expect(await this.Ticket.WINNER_PICKER()).to.equal(
          this.WINNER_PICKER_MOCK.address
        );
      });
    });
    ...
    describe("Input validation", async function () {
      afterEach(async function () {
        await expect(this.Ticket.initialize(...this.PARAMS)).to.be.revertedWith(
          "InvalidInput()"
        );
      });
  ...
  describe("Ticket purchasing", async function () {
    describe("Restrictions", async function () {
      it("should not be allowed before starting block", async function () {
        ...
        await expect(
          this.Ticket.buyTicket({ value: PRICE })
        ).to.be.revertedWith("Unavailable()");
        await expect(
          this.Ticket.buyTicketWithURI("uri", { value: PRICE })
        ).to.be.revertedWith("Unavailable()");
      });

      it("should not be allowed after end block", async function () {
        const blocksUntilEnd =
          this.BLOCKS.END_BLOCK - this.BLOCKS.CURRENT_BLOCK;
      ...
      it("should check if sending ether amount corresponding to the ticket price (normal)", async function () {
        this.PARAMS[2] = this.BLOCKS.CURRENT_BLOCK + 1;
        await this.Ticket.initialize(...this.PARAMS);
        await expect(this.Ticket.buyTicket({ value: 1 })).to.be.revertedWith(
          "InvalidAmount()"
        );
        await expect(
          this.Ticket.buyTicket({ value: PRICE })
        ).to.not.be.revertedWith("InvalidAmount()");
      });
      ...
      it("should check if sending ether amount corresponding to the ticket price (uri)", async function () {
        ...
        await this.Ticket.initialize(...this.PARAMS);
        await expect(
          this.Ticket.buyTicketWithURI("uri", { value: 1 })
  ...
  describe("Picking winner", async function () {
    beforeEach(async function () {
      await this.Ticket.initialize(...this.PARAMS);
      this.blocksUntilMidtimeBlockNumber =
        this.BLOCKS.START_BLOCK -
        this.BLOCKS.CURRENT_BLOCK +
        (this.BLOCKS.END_BLOCK - this.BLOCKS.START_BLOCK) / 2 -
        2;
    });
    ...
    describe("Block validations", async function () {
      it("should not be able to pick the small winner before midtime", async function () {
        ...
        await expect(this.Ticket.pickWinner()).to.be.revertedWith(
          "Unavailable()"
        );
      });
      ...
      it("should be able to pick the small winner before end", async function () {
        ...
        await expect(this.Ticket.pickWinner()).to.not.be.revertedWith(
          "Unavailable()"
        );
      });
    ...
    describe("VRFConsumer/WinnerPicker contract interaction", async function () {
      ...
      it("should request a random number", async function () {
        ...
        const signatureSmall = "pickSmallWinner(uint256)";
        const signatureBig = "pickBigWinner(uint256)";
        await this.WINNER_PICKER_MOCK.mock.getRandomNumber
          .withArgs(signatureSmall)
          .revertsWithReason(signatureSmall);
        await this.WINNER_PICKER_MOCK.mock.getRandomNumber
          .withArgs(signatureBig)
          .revertsWithReason(signatureBig);
      ...
      it("should save boolean that we have picked a winner", async function () {
        ...
        expect(await this.Ticket.pickedSmallWinner()).to.be.true;

        await mineBlocks(this.blocksUntilMidtimeBlockNumber);
        await this.Ticket.pickWinner();
        expect(await this.Ticket.pickedBigWinner()).to.be.true;
      });
    });
  });
  ...
  describe("Choosing winners", async function () {
    ...
    describe("Saving small winner", async function () {
      it("should save winning ticket id", async function () {
        await this.Ticket.pickSmallWinner(this.randomness);
      ...
      it("should save reward amount equal to half of current balance", async function () {
        await this.Ticket.pickSmallWinner(this.randomness);
        expect(await this.Ticket.smallWinnerRewardAmount()).to.equal(
          0.5 * Number(await ethers.provider.getBalance(this.Ticket.address))
        );
      });

      it("should emit an event", async function () {
        await expect(this.Ticket.pickSmallWinner(this.randomness))
          .to.emit(this.Ticket, "WinnerChoosen")
    ...
    describe("Saving big winner", async function () {
      beforeEach(async function () {
        await mineBlocks(
          this.BLOCKS.END_BLOCK - this.BLOCKS.START_BLOCK + 1000
        );
      });

      it("should save winning ticket id", async function () {
        await this.Ticket.pickBigWinner(this.randomness);
        expect(await this.Ticket.bigWinnerTicketId()).to.equal(1); // 3 % 2 = 1
      });

      it("should emit an event", async function () {
        await this.Ticket.pickBigWinner(this.randomness);
        await expect(this.Ticket.pickBigWinner(this.randomness))
          .to.emit(this.Ticket, "WinnerChoosen")
          .withArgs(deployer.address, 1);
      });
    });
